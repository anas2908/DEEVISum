...
class CustomDataset(Dataset):
    """
    PyTorch Dataset for video classification using 1fps frames and metadata from Excel.
    """
    def __init__(self, excel_path: str = "TVSum processed.xlsx", video_root: str = "video_data", prompt_file: str = "prompt_template.txt", transform=None):
        super().__init__()
    ...
    def _prepare_samples(self) -> List[Tuple[Image.Image, str, int]]:
        samples = []
        for _, row in self.df.iterrows():
            video_id = str(row['video_id']).strip()
            video_path = os.path.join(self.video_root, f"{video_id}.mp4")

            # Load 1fps frames from video
            frames = self._extract_frames(video_path, fps=1)

            # Parse gold labels
            gold_labels = [int(x.strip()) for x in str(row['ogp_1fps(3,4,5)']).split(',')]

            # Construct metadata prompt
            meta_prompt = (
                self.prompt_template + "\n" +
                f"Title: {row['title']}\n" +
                f"Transcript: {row['transcript_processed']}\n" +
                f"Audio Summary: {row['clubbed_audio_processed']}"
            )

            # Zip each frame with its corresponding label and metadata
            for i, frame in enumerate(frames):
                if i < len(gold_labels):
                    samples.append((frame, meta_prompt, gold_labels[i]))
                else:
                    break  # Stop if gold labels are fewer than frames
        return samples

    def _extract_frames(self, video_path: str, fps: int = 1) -> List[Image.Image]:
        frames = []
        ...
        frame_interval = int(round(frame_rate / fps)) if frame_rate > 0 else 1
    ...
    def __getitem__(self, idx: int):
        image, prompt, label = self.samples[idx]
        if self.transform:
...
class KnowledgeDistillationVLM(L.LightningModule):
    def __init__(self, config, processor, student_model, mentor_model, teacher_model, train_dataset, val_dataset):
        ...
        self.kd_weight_long = torch.nn.Parameter(torch.tensor(0.25, requires_grad=True))  # For Teacher → Student
    ...
    def training_step(self, batch, batch_idx):
        ...
        mentor_loss = mentor_outputs.loss
        ...
        total_mentor_loss = mentor_loss + kd_loss_mentor * self.kd_weight_mentor
        total_student_loss = student_loss + kd_loss_student * self.kd_weight_student + kd_loss_long * self.kd_weight_long
        ...
        # ===== EXIT VECTORS CALCULATION FOR STUDENT MODEL ONLY =====
        from PIL import Image
        import pandas as pd
        import numpy as np
        ...
        df = pd.read_excel(self.config["exit_vector_excel_path"])
        DEVICE = self.config.get("device", "cuda")
        model = self.student_model
        processor = self.processor
        EXIT_LAYERS = self.config.get("exit_layers", [2, 4, 6])
...
config = {"max_epochs": 20, "gradient_clip_val": 1.0, "accumulate_grad_batches": 8, "lr": 1e-4, "batch_size": 2}
